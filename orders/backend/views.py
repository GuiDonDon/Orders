...
from .PartnerYAMLImporter import PartnerYAMLImporter
...
def import_yaml_view(request):
    source = 'shop1.yaml'  # или можешь принимать путь через request.POST
    importer = PartnerYAMLImporter(user_id=request.user.id, source=source)
...
class PartnerUpdate(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            importer = PartnerYAMLImporter(partner=request.user.id, source=source)
            importer.load()
            result = importer.import_data()
            return JsonResponse(result)
        except Exception as e:
